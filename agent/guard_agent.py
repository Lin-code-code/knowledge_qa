...
class GuardAgent:
    ...
    def check_question_scope(self, query: str) -> bool:
        ...
        try:
            ...
            logger.error(f"[GuardAgent] 域内预检调用失败，默认放行: {str(e)}")
            return True

        label = self._parse_scope_label(raw)
        logger.info(f"[GuardAgent] scope_check query={query!r} label={label}")
        return label == "IN"
    ...
    def _parse_scope_label(raw: str) -> str:
        ...
        if "NO" in text:
            return "OUT"
        logger.warning(f"[GuardAgent] scope label 解析失败，默认 IN: {text[:120]}")
        return "IN"
    ...
    def check(self, query: str, answer: str) -> tuple[bool, str]:
        ...
        try:
            ...
            logger.error(f"[GuardAgent] L3 分类器调用失败，默认放行: {str(e)}")
            return True, answer

        label = self._parse_label(raw)
        logger.info(f"[GuardAgent] L3 query={query!r} label={label}")
    ...
    def _parse_label(raw: str) -> str:
        ...
        logger.warning(f"[GuardAgent] label 解析失败，默认 IN: {text[:120]}")
        return "IN"
